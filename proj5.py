...
def login():
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        if account:
            ...
            return render_template('home.html', inc="Please fill up the form!", inc2="")
    ...
    elif request.method == 'POST' and 'regusername' in request.form and 'regpassword' in request.form and 'regemail' in request.form:
        # Create variables for easy access
        ...
        if account:
            ...
            flash("Username must only contain characters or numbers!", "error")
        ...
        flash("Please fill up the form!", "error")
        return render_template('home.html', inc="Please fill up the form!", inc2="Please fill up the form!")
...
def home(name, result=[]):
    ...
    battcols=['ID', 'Name', 'Description', 'Armor Lost, Axis', 'Aircraft Lost, Axis', 'Carriers Lost, Axis', 'Battleships Lost, Axis', 
            ...
            'Carriers Lost, Allies', 'Battleships Lost, Allies', 'Cruisers Lost, Allies', 'Destroyers Lost, Allies', 'Submarines Lost, Allies',
    ...
    countrycols=['Name', 'Max Ground Strength', 'No. of Armor', 'No. of Carriers', 'No. of Battleships', 'No. of Cruisers', 'No. of Destroyers', 'No. of Submarines',
    ...
    countryvar=['NAME', 'MAXGSTR', 'NUMARMOR', 'NUMCV', 'NUMBB', 'NUMC', 'NUMDD', 'NUMSS', 'NUMFIGHT', 'NUMBOMB', 'NUMARMORL', 'NUMAIRCRAFTL', 'NUMCVL',
    ...
    #new
    axisforcecols=['Country', 'Military name', 'Axis ID']
    axisforcevar=['MCNAME', 'MNAME', 'AXISKEY']
    alliedforcecols=['Country', 'Military name', 'Allied ID']
    alliedforcevar=['MCNAME', 'MNAME', 'ALLIEDKEY']
    axiscols=['ID']
    axisvar=['ID']
    alliedcols=['ID']
    alliedvar=['ID']

    colsdictionary={'Battle':battcols, 'Commander':comcols, 'COMMANDM':commandmcols, 'Military under Command':commandmcols, 'Country':countrycols, 'Event':eventcols, 'has':hascols, 'Battle Info':hascols, 'Ship Info':hasscols, 'hass':hasscols, 'Military':milcols, 'Place':placecols, 'Ship':shipcols, 'Surrender':surrendercols, 'War':warcols, 'AxisForce':axisforcecols, 'AlliedForce':alliedforcecols, 'Axis Force':axisforcecols, 'Allied Force':alliedforcecols,'Axis':axiscols, 'Allies':alliedcols}
    vardictionary={'Battle':battvar, 'Commander':comvar, 'COMMANDM':commandmvar, 'Military under Command':commandmvar, 'Country':countryvar, 'Event':eventvar, 'has':hasvar, 'Battle Info':hasvar, 'Ship Info':hassvar,'hass':hassvar, 'Military':milvar, 'Place':placevar, 'Ship':shipvar, 'Surrender':surrendervar, 'War':warvar, 'AxisForce':axisforcevar, 'AlliedForce':alliedforcevar, 'Axis Force':axisforcevar, 'Allied Force':alliedforcevar, 'Axis':axisvar, 'Allies':alliedvar}
    labelsdictionary={'Battle':'Battle', 'Commander':'Commander', 'COMMANDM':'Military under Command', 'Country':'Country', 'Event':'Event', 'has':'Battle Info', 'hass':'Ship Info', 'Military':'Military', 'Place':'Place', 'Ship':'Ship', 'Surrender':'Surrender', 'War':'War', 'AxisForce':'Axis Force', 'AlliedForce':'Allied Force', 'Axis Force':'Axis Force', 'Allied Force':'Allied Force', 'Axis':'Axis', 'Allies':'Allies'}
    PKdictionary={'Battle':battvar[0:1], 'Commander':comvar[0:1], 'COMMANDM':commandmvar[0:3], 'Military under Command':commandmvar[0:3], 'Country':countryvar[0:1], 'Event':eventvar[0:1], 'has':hasvar[0:4], 'Battle Info':hasvar[0:4], 'Ship Info':hassvar[0:3],'hass':hassvar[0:3], 'Military':milvar[0:2], 'Place':placevar[0:1], 'Ship':shipvar[0:1], 'Surrender':surrendervar[0:1], 'War':warvar[0:2], 'AxisForce':axisforcevar[:], 'AlliedForce':alliedforcevar[:], 'Axis Force':axisforcevar[:], 'Allied Force':alliedforcevar[:], 'Axis':axisvar[:], 'Allies':alliedvar[:]}
    ...
    countryvariablenames=['Max Ground Strength', 'No. of Armor', 'No. of Carriers', 'No. of Battleships', 'No. of Cruisers', 'No. of Destroyers', 'No. of Submarines',
        'No. of Fighters', 'No. of Bombers', 'Armor Lost', 'Aircraft Lost', 'Carriers Lost', 'Battleships Lost', 'Cruisers Lost', 'Destroyers Lost',
        'Submarines Lost', 'Military Casualties', 'Civilian Casualties']
    countryvariable=['MAXGSTR', 'NUMARMOR', 'NUMCV', 'NUMBB', 'NUMC', 'NUMDD', 'NUMSS', 'NUMFIGHT', 'NUMBOMB', 'NUMARMORL', 'NUMAIRCRAFTL', 'NUMCVL',
        'NUMBBL', 'NUMCL', 'NUMDDL', 'NUMSSL', 'MCASUALTY', 'CCASUALTY']
    cols, var, PK=[], [], []
    tablename=''
    #break
    ...
    if request.method=='POST' and 'search' in request.form:
        ...
        #return render_template('name4.html', name=name, battlelist=battlelist, numBattle=len(battlelist), countrylist=countrylist, numCountry=len(countrylist), countryvariablenames=countryvariablenames, countryvariable=countryvariable, numCountryVariable=len(countryvariable), result=result, numResult=len(result), cols=cols, var=var, numCols=len(var), tablename=tablename, PK=PK)
        return render_template('name5.html', name=name, battlelist=battlelist, numBattle=len(battlelist), countrylist=countrylist, numCountry=len(countrylist), countryvariablenames=countryvariablenames, countryvariable=countryvariable, numCountryVariable=len(countryvariable), result=result, numResult=len(result), cols=cols, var=var, numCols=len(var), tablename=tablename, PK=PK, result1=result1, result2=result2, result3=result3, result4=result4, results=results, numResults=len(results), columnName=cols, varName=var, tab=tab, colsplotted=colsplotted, imgs=imgs, numimgs=len(imgs), tablestats=tablestats, tablestatscols=tablestatscols, numStats=len(tablestats), loaddata=loaddata)
    ...
    #return render_template('name4.html', name=name, battlelist=battlelist, numBattle=len(battlelist), countrylist=countrylist, numCountry=len(countrylist), countryvariablenames=countryvariablenames, countryvariable=countryvariable, numCountryVariable=len(countryvariable), result=result, numResult=len(result), cols=cols, var=var, numCols=len(var), tablename=tablename, PK=PK)
    elif request.method=='POST' and 'search7' in request.form and 'tables7' in request.form:
        search7=request.form['search7']
        table7 = request.form.getlist('tables7')
        by7=request.form.getlist('by7')
        sort7 = request.form.getlist('sort7')
        order7 = request.form.getlist('order7')
        params=[search7,table7,by7,sort7,order7]
        tab=7
    ...
    if tab==1:
        ...
        if cat=='Battle':
        ...
        elif cat=='Ship': #order by, filter columns sort by 2+ columns
            if by==1: #country
                cursor.execute('SELECT * FROM {} WHERE CNAME LIKE {}'.format(cat, '\'%'+search+'%\''))
            elif by==2: #name
                cursor.execute('SELECT * FROM {} WHERE NAME LIKE {}'.format(cat, '\'%'+search+'%\''))
            ...
            if by==1: #Country
                cursor.execute('SELECT * FROM {} WHERE C1 LIKE {} OR C2 LIKE {}'.format(cat, '\'%'+search+'%\'', '\'%'+search+'%\''))
            elif by==2: #Date
                cursor.execute('SELECT * FROM {} WHERE SDATE=CAST({} AS DATE)'.format(cat, '\''+search+'\''))
            else: #Description
                cursor.execute('SELECT * FROM {} WHERE DESCR LIKE {}'.format(cat, '\'%'+search+'%\''))
        elif cat=='Country':
            if by==1: #>
                #print('SELECT * FROM COUNTRY WHERE COUNTRY.MAXGSTR<(SELECT MAXGSTR FROM COUNTRY WHERE COUNTRY.NAME LIKE {})'.format('\'%'+search+'%\''))
                cursor.execute('SELECT * FROM COUNTRY WHERE COUNTRY.MAXGSTR<(SELECT MAXGSTR FROM COUNTRY WHERE COUNTRY.NAME LIKE {})'.format('\'%'+search+'%\''))
            elif by==2: #<
                cursor.execute('SELECT * FROM COUNTRY WHERE COUNTRY.MAXGSTR>(SELECT MAXGSTR FROM COUNTRY WHERE COUNTRY.NAME LIKE {})'.format('\'%'+search+'%\''))
            #by3,by4
        results = cursor.fetchall()
        return render_template('name5.html', name=name, battlelist=battlelist, numBattle=len(battlelist), countrylist=countrylist, numCountry=len(countrylist), countryvariablenames=countryvariablenames, countryvariable=countryvariable, numCountryVariable=len(countryvariable), result=result, numResult=len(result), cols=cols, var=var, numCols=len(var), tablename=tablename, PK=PK, result1=result1, result2=result2, result3=result3, result4=result4, results=results, numResults=len(results), columnName=cols, varName=var, tab=tab, colsplotted=colsplotted, imgs=imgs, numimgs=len(imgs), tablestats=tablestats, tablestatscols=tablestatscols, numStats=len(tablestats), loaddata=loaddata)
        #should have added place variable to event, more complete
    elif tab==2: #select/filter by columns? show only certain columns? need another variable + how to do that? filter by column name + filter by column data dynamic search bar
        #how about sorting by derived attributes eg total casualties instead of axis or allied casualties
    ...
    #event lists, surrender war,format stats data, table width, create checkbox 'all',
        ...
        for i in range(1,len(alldata),num):
            ...
            if temp[-1]=='0001': #nochange
                ...
                if alldata[0]!='Axis' and alldata[0]!='Allies' and alldata[0]!='Axis Force' and alldata[0]!='Allied Force':
                    #print("here")
                ...
                cursor.execute('INSERT INTO {} VALUES {}'.format(alldata[0], insertstatement))
                mysql.connection.commit()
        ...
        #print(params)
        search7, table7, by7, sort7, order7 = params[0], params[1], params[2], params[3], params[4]
        cols=colsdictionary[table7[0]]
        var=vardictionary[table7[0]]

        #sorts = cols[int(sort[0])-1]

        bys7 = var[int(by7[0])-1]
        sorts7 = var[int(sort7[0])-1]

        if(search7=='None'):
            search7='NULL'
            #print('SELECT * FROM {} WHERE {} IS NULL ORDER BY {} {}'.format(table7[0], bys7, sorts7, order7[0]))
            cursor.execute('SELECT * FROM {} WHERE {} iS NULL ORDER BY {} {}'.format(table7[0], bys7, sorts7, order7[0]))
            results=cursor.fetchall()
            #print(results)
        else:
            try:
                #print('SELECT * FROM {} WHERE {}={} ORDER BY {} {}'.format(table7[0], bys7, int(search7), sorts7, order7[0]))
                cursor.execute('SELECT * FROM {} WHERE {}={} ORDER BY {} {}'.format(table7[0], bys7, int(search7), sorts7, order7[0]))
                results=cursor.fetchall()
                #print(results)
            except:
                try:
                    if('DATE' in bys7):
                        #print('SELECT * FROM {} WHERE {}=CAST({} AS DATE) ORDER BY {} {}'.format(table7[0], bys7, '\''+search7+'\'', sorts7, order7[0]))
                        cursor.execute('SELECT * FROM {} WHERE {}=CAST({} AS DATE) ORDER BY {} {}'.format(table7[0], bys7, '\''+search7+'\'', sorts7, order7[0]))
                        results=cursor.fetchall()
                        #print(results)
                    else:
                        #print('SELECT * FROM {} WHERE {} LIKE {} ORDER BY {} {}'.format(table7[0], bys7, '\'%'+search7+'%\'', sorts7, order7[0]))
                        cursor.execute('SELECT * FROM {} WHERE {} LIKE {} ORDER BY {} {}'.format(table7[0], bys7, '\'%'+search7+'%\'', sorts7, order7[0]))
                        results=cursor.fetchall()
                        #print(results)
                except:
                    pass
        ...
        #cursor.execute('SELECT * FROM {} ORDER BY {} {}'.format(table7[0], sorts7, order7[0]))
        #results = cursor.fetchall()
        #return render_template('name5.html', name=name, battlelist=battlelist, numBattle=len(battlelist), countrylist=countrylist, numCountry=len(countrylist), countryvariablenames=countryvariablenames, countryvariable=countryvariable, numCountryVariable=len(countryvariable), result=result, numResult=len(result), cols=cols, var=var, numCols=len(var), tablename=tablename, PK=PK, result1=result1, result2=result2, result3=result3, result4=result4, results=results, numResults=len(results), columnName=cols, varName=var, tab=tab, colsplotted=colsplotted, imgs=imgs, numimgs=len(imgs), tablestats=tablestats, tablestatscols=tablestatscols, numStats=len(tablestats), loaddata=loaddata)
    return render_template('name5.html', name=name, battlelist=battlelist, numBattle=len(battlelist), countrylist=countrylist, numCountry=len(countrylist), countryvariablenames=countryvariablenames, countryvariable=countryvariable, numCountryVariable=len(countryvariable), result=result, numResult=len(result), cols=cols, var=var, numCols=len(var), tablename=tablename, PK=PK, result1=result1, result2=result2, result3=result3, result4=result4, results=results, numResults=len(results), columnName=cols, varName=var, tab=tab, colsplotted=colsplotted, imgs=imgs, numimgs=len(imgs), tablestats=tablestats, tablestatscols=tablestatscols, numStats=len(tablestats), loaddata=loaddata)
...
def search_results(name, tab, params, battlelist, countrylist, countryvariablenames, countryvariable):
    ...
    colsdictionary={'Battle':battcols, 'Commander':comcols, 'COMMANDM':commandmcols, 'Country':countrycols, 'Event':eventcols, 'has':hascols, 'hass':hasscols, 'Military':milcols, 'Place':placecols, 'Ship':shipcols, 'Surrender':surrendercols, 'War':warcols, 'axisforce':axisforcecols, 'alliedforce':alliedforcecols, 'axis':axiscols, 'allies':alliedcols}
    vardictionary={'Battle':battvar, 'Commander':comvar, 'COMMANDM':commandmvar, 'Country':countryvar, 'Event':eventvar, 'has':hasvar, 'hass':hassvar, 'Military':milvar, 'Place':placevar, 'Ship':shipvar, 'Surrender':surrendervar, 'War':warvar, 'axisforce':axisforcevar, 'alliedforce':alliedforcevar, 'axis':axisvar, 'allies':alliedvar}
    labelsdictionary={'Battle':'Battle', 'Commander':'Commander', 'COMMANDM':'Military under Command', 'Country':'Country', 'Event':'Event', 'has':'Battle Info', 'hass':'Ship Info', 'Military':'Military', 'Place':'Place', 'Ship':'Ship', 'Surrender':'Surrender', 'War':'War', 'axisforce':'Axis Force', 'alliedforce':'Allied Force', 'axis':'Axis', 'allies':'Allies'}
    PKdictionary={'Battle':battvar[0:1], 'Commander':comvar[0:1], 'COMMANDM':commandmvar[0:3], 'Country':countryvar[0:1], 'Event':eventvar[0:1], 'has':hasvar[0:4], 'hass':hassvar[0:3], 'Military':milvar[0:2], 'Place':placevar[0:1], 'Ship':shipvar[0:1], 'Surrender':surrendervar[0:1], 'War':warvar[0:2], 'axisforce':axisforcevar[:], 'alliedforce':alliedforcevar[:], 'axis':axisvar[:], 'allies':alliedvar[:]}
    ...
    #return render_template('name4.html', name=name, results=results, numResults=len(results), columnName=cols, varName=var, numCols=len(cols), tab=tab, result1=result1, result2=result2, result3=result3, result4=result4, colsplotted=colsplotted, imgs=imgs, numimgs=len(imgs), battlelist=battlelist, numBattle=len(battlelist), countrylist=countrylist, numCountry=len(countrylist), tablestats=tablestats, tablestatscols=tablestatscols, numStats=len(tablestats), countryvariablenames=countryvariablenames, countryvariable=countryvariable, numCountryVariable=len(countryvariable))
    return render_template('name4.html', name=name, results=results, numResults=len(results), columnName=cols, varName=var, numCols=len(cols), tab=tab, result1=result1, result2=result2, result3=result3, result4=result4, colsplotted=colsplotted, imgs=imgs, numimgs=len(imgs), battlelist=battlelist, numBattle=len(battlelist), countrylist=countrylist, numCountry=len(countrylist), tablestats=tablestats, tablestatscols=tablestatscols, numStats=len(tablestats), countryvariablenames=countryvariablenames, countryvariable=countryvariable, numCountryVariable=len(countryvariable), result=result, numResult=len(result), tablename=tablename, PK=PK, cols=cols, var=var, loaddata=loaddata)
