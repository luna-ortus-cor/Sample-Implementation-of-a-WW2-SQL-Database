...
def home(name, result=[]):
    ...
    if request.method=='POST' and 'search' in request.form:
    ...
    elif request.method=='POST' and 'up' in request.form:
        ...
        return render_template('name.html', name=name, battlelist=battlelist, numBattle=len(battlelist), countrylist=countrylist, numCountry=len(countrylist), countryvariablenames=countryvariablenames, countryvariable=countryvariable, numCountryVariable=len(countryvariable), result=result, numResult=len(result), cols=cols, var=var, numCols=len(var), tablename=tablename)
    elif request.method=='POST' and 'edits' in request.form:
...
def search_results(name, tab, params, battlelist, countrylist, countryvariablenames, countryvariable):
    ...
    #event lists, surrender war,format stats data, table width, create checkbox 'all',
        ...
        print(edit)
        edits=edit.split(',')
        cols=colsdictionary[edits[0]]
        var=vardictionary[edits[0]]
        num=len(var)
        for i in range(1,len(edits),num):
            s=''
            for j in edits[i:i+num]:
                s+=str(j)[1:-1]
                s+=','
            s=s[:-1]
            s=str(s)
            ...
            s='('+s+')'
            print(s)
            cursor.execute('INSERT INTO {} VALUES {}'.format(edits[0], s))
            mysql.connection.commit()
